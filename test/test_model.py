...
说明：
    当前测试依赖 PyTorch。如果测试环境没有安装 torch，pytest 会自动跳过本文件。
"""
...
pytestmark = pytest.mark.skipif(not TORCH_AVAILABLE, reason="需要安装 PyTorch 才能测试模型")
...
def test_attention_masks_shape_and_values() -> None:
    ...
    LOGGER.info("tokens=%s padding_idx=0", tokens.tolist())
    LOGGER.info("attn_mask=%s", attn_mask.int().tolist())
    LOGGER.info("pad_mask=%s", pad_mask.int().tolist())
    ...
    assert pad_mask.tolist() == expected_pad_mask, (
        f"padding mask mismatch: expected {expected_pad_mask}, got {pad_mask.tolist()}"
    )


def test_json_output_template_renders_field_formats() -> None:
...
def test_json_output_head_decodes_hidden_states() -> None:
    """测试 JSON 输出头能把 hidden states 解码为 JSON 对象。"""
    torch.manual_seed(0)

    output_head = JsonOutputHead(d_model=16, output_template=_json_action_template())
...
def test_decoder_config_rejects_invalid_heads() -> None:
    """d_model 不能被 num_heads 整除时，配置应直接报错。"""
    LOGGER.info("reject invalid decoder config d_model=30 num_heads=8")
...
def test_transformer_decoder_forward_shape() -> None:
    ...
    memory = torch.randn(2, 5, config.d_model)
    LOGGER.info("decoder input_ids shape=%s sample=%s", tuple(input_ids.shape), input_ids[0].tolist())
...
def test_transformer_decoder_return_dict() -> None:
    ...
    input_ids = torch.randint(1, config.vocab_size, (2, 5))
    memory = torch.randn(2, 7, config.d_model)
    LOGGER.info("return_dict input_ids shape=%s sample=%s", tuple(input_ids.shape), input_ids[0].tolist())
...
def test_image_encoder_rejects_more_than_ten_images() -> None:
    """图片数量超过 10 张时，应抛出错误。"""
    ...
    encoder = ImageTokenEncoder.from_config(config)
    encoder.eval()

    images = torch.randn(2, 11, 3, 32, 32)
    LOGGER.info("too many images shape=%s max_images=%d", tuple(images.shape), config.max_images)
...
def test_multimodal_model_forward_with_images_and_text() -> None:
    ...
    images = torch.randn(batch_size, num_images, 3, 32, 32)
    ...
    LOGGER.info("text_input_ids shape=%s sample=%s", tuple(text_input_ids.shape), text_input_ids[0].tolist())
    LOGGER.info(
        "decoder_input_ids shape=%s sample=%s",
...
def test_multimodal_model_return_json_output() -> None:
...
def test_multimodal_model_return_memory_and_masks() -> None:
    ...
    decoder_input_ids = torch.randint(1, action_vocab_size, (batch_size, target_len))
    LOGGER.info("image_padding_mask=%s", image_padding_mask.int().tolist())
    LOGGER.info("text_input_ids=%s", text_input_ids.tolist())
    LOGGER.info("decoder_input_ids shape=%s", tuple(decoder_input_ids.shape))
    ...
    LOGGER.info("expected_memory_key_padding_mask=%s", expected_mask.int().tolist())
    LOGGER.info("actual_memory_key_padding_mask=%s", output.memory_key_padding_mask.int().tolist())
    assert torch.equal(output.memory_key_padding_mask, expected_mask), (
        "memory_key_padding_mask mismatch: "
        f"expected {expected_mask.tolist()}, got {output.memory_key_padding_mask.tolist()}"
    )
